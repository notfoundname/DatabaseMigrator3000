...
public class Config extends YamlSerializable {
  ...
  public Map<String, NamedQuery> presets = Map.of(
      ...
      "nlogin_old", new NamedQuery("nLogin Old", "SELECT realname, name, COALESCE(password, \"\"), COALESCE(address, \"\"), \"\", 0, CONCAT(SUBSTR(uniqueId, 1, 8), '-', SUBSTR(uniqueId, 9, 4), '-', SUBSTR(uniqueId, 13, 4), '-',  SUBSTR(uniqueId, 17, 4), '-', SUBSTR(uniqueId, 21)), IF(premiumId IS NULL, \"\", CONCAT(SUBSTR(premiumId, 1, 8), '-', SUBSTR(premiumId, 9, 4), '-', SUBSTR(premiumId, 13, 4), '-',  SUBSTR(premiumId, 17, 4), '-', SUBSTR(premiumId, 21))), NULL, NULL, NULL FROM table"),
      "nlogin_new", new NamedQuery("nLogin New", "SELECT last_name, LOWER(last_name), COALESCE(password, \"\"), COALESCE(last_ip, \"\"), NULL, creation_date, CONCAT(SUBSTR(unique_id, 1, 8), '-', SUBSTR(unique_id, 9, 4), '-', SUBSTR(unique_id, 13, 4), '-',  SUBSTR(unique_id, 17, 4), '-', SUBSTR(unique_id, 21)), IF(mojang_id IS NULL, \"\", CONCAT(SUBSTR(mojang_id, 1, 8), '-', SUBSTR(mojang_id, 9, 4), '-', SUBSTR(mojang_id, 13, 4), '-',  SUBSTR(mojang_id, 17, 4), '-', SUBSTR(mojang_id, 21))), COALESCE(last_ip, \"\"), last_seen, NULL FROM nlogin WHERE last_name IS NOT NULL"),
